...
public class Card : MonoBehaviour
{
    ...
    {
        enum State { normal, hover, posthover, clicked };
        State state;

        static Dictionary<State, State> UpdateTransition = new Dictionary<State, State>()
        {{State.hover,State.posthover}, {State.posthover,State.normal}, {State.normal,State.normal}, {State.clicked,State.clicked}};

        static Dictionary<State, bool> QueryFunction = new Dictionary<State, bool>()
        {{State.hover,true}, {State.posthover,true}, {State.normal,false}, {State.clicked, true}};
        ...
        public void SetClick(bool command)
        {
            if (command)
            {
                state = State.clicked;
    ...
    public bool click { get { return hover; } set { _hover.SetClick(value); } }
    public int number
    {
